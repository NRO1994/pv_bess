...
def _write_price_csv(
    ...
    df.to_csv(path, index=False, sep=CSV_DELIMITER)
...
def _write_price_csv_constant(
    ...
    for col, val in columns.items():
        data[col] = [val] * n_rows
    pd.DataFrame(data).to_csv(path, index=False, sep=CSV_DELIMITER)
...
class TestLoadMarketPricesValidation:
    ...
    def test_too_few_rows_raises_value_error(self, tmp_path: Path) -> None:
        ...
        pd.DataFrame({"timestamp": timestamps, "MID": [50.0] * 100}).to_csv(csv, index=False, sep=CSV_DELIMITER)
